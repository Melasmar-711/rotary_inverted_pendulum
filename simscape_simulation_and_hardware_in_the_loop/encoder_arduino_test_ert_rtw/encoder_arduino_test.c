...
void encoder_arduino_test_step(void)
{
   ...
   *  Sum: '<Root>/Sum'
   */
  rtb_Saturation_h = (((encoder_arduino_test_P.refernce_values[0] - rtb_Gain3) *
   ...
   *  Gain: '<Root>/Gain'
   */
  if (rtb_Saturation_h > encoder_arduino_test_P.Saturation_UpperSat_e) {
    rtb_Saturation_h = encoder_arduino_test_P.Saturation_UpperSat_e;
  } else {
    if (rtb_Saturation_h < encoder_arduino_test_P.Saturation_LowerSat_j) {
      rtb_Saturation_h = encoder_arduino_test_P.Saturation_LowerSat_j;
    }
  }
  ...
  /* Signum: '<Root>/Sign' */
  if (rtb_Saturation_h < 0.0) {
    rtb_Sign_i = -1.0;
  } else if (rtb_Saturation_h > 0.0) {
    rtb_Sign_i = 1.0;
  } else if (rtb_Saturation_h == 0.0) {
    rtb_Sign_i = 0.0;
  } else {
  ...
  if (rtb_Sign_i > encoder_arduino_test_P.Switch_Threshold_b) {
    ...
    if (rtb_Saturation_h > encoder_arduino_test_P.Saturation1_UpperSat) {
      rtb_Saturation_h = encoder_arduino_test_P.Saturation1_UpperSat;
    } else {
      if (rtb_Saturation_h < encoder_arduino_test_P.Saturation1_LowerSat) {
        rtb_Saturation_h = encoder_arduino_test_P.Saturation1_LowerSat;
      }
    }

    /* End of Saturate: '<Root>/Saturation1' */
    rtb_Saturation_h *= encoder_arduino_test_P.Gain1_Gain;
  } else {
    if (rtb_Saturation_h > encoder_arduino_test_P.Saturation2_UpperSat) {
      /* Saturate: '<Root>/Saturation2' */
      rtb_Saturation_h = encoder_arduino_test_P.Saturation2_UpperSat;
    } else {
      if (rtb_Saturation_h < encoder_arduino_test_P.Saturation2_LowerSat) {
        /* Saturate: '<Root>/Saturation2' */
        rtb_Saturation_h = encoder_arduino_test_P.Saturation2_LowerSat;
      }
    }

    rtb_Saturation_h *= encoder_arduino_test_P.Gain2_Gain;
  }
  ...
  /* Gain: '<Root>/Gain3' */
  rtb_Saturation_h *= encoder_arduino_test_P.Gain3_Gain_c;

  /* MATLABSystem: '<Root>/PWM' */
  if (!(rtb_Saturation_h < 255.0)) {
    rtb_Saturation_h = 255.0;
  }
  ...
  MW_PWM_SetDutyCycle(encoder_arduino_test_DW.obj.MW_PWM_HANDLE,
                      rtb_Saturation_h);
  ...
  if (rtb_Sign_i > encoder_arduino_test_P.Switch1_Threshold_n) {
    rtb_Saturation_h = encoder_arduino_test_P.Constant1_Value;
  } else {
    rtb_Saturation_h = encoder_arduino_test_P.Constant2_Value;
  }

  /* End of Switch: '<Root>/Switch1' */

  /* MATLABSystem: '<Root>/Digital Output' */
  rtb_Sign_i = rt_roundd_snf(rtb_Saturation_h);
  if (rtb_Sign_i < 256.0) {
    if (rtb_Sign_i >= 0.0) {
   ...
   *  Logic: '<Root>/NOT'
   */
  writeDigitalPin(8, (uint8_T)!(rtb_Saturation_h != 0.0));
...
void encoder_arduino_test_initialize(void)
{
  ...
  /* External mode info */
  encoder_arduino_test_M->Sizes.checksums[0] = (2234704417U);
  encoder_arduino_test_M->Sizes.checksums[1] = (3412799635U);
  encoder_arduino_test_M->Sizes.checksums[2] = (3539478939U);
  encoder_arduino_test_M->Sizes.checksums[3] = (2979686149U);
