...
void encoder_arduino_test_step(void)
{
  ...
  if (encoder_arduino_test_DW.obj_n.SampleTime !=
      encoder_arduino_test_P.MATLABSystem2_SampleTime) {
    encoder_arduino_test_DW.obj_n.SampleTime =
      encoder_arduino_test_P.MATLABSystem2_SampleTime;
  }

  encoder_arduino_test_B.MATLABSystem2 = enc_output(0.0);

  /* End of MATLABSystem: '<S1>/MATLAB System2' */

  /* Gain: '<S1>/Gain' */
  encoder_arduino_test_B.Gain = (real_T)encoder_arduino_test_P.Gain_Gain *
    2.3283064365386963E-10 * (real_T)encoder_arduino_test_B.MATLABSystem2;
...
void encoder_arduino_test_initialize(void)
{
  ...
  /* External mode info */
  encoder_arduino_test_M->Sizes.checksums[0] = (2061290338U);
  encoder_arduino_test_M->Sizes.checksums[1] = (3975826406U);
  encoder_arduino_test_M->Sizes.checksums[2] = (1639986270U);
  encoder_arduino_test_M->Sizes.checksums[3] = (3029024883U);
...

  {
    ...
    /* Start for MATLABSystem: '<S1>/MATLAB System2' */
    encoder_arduino_test_DW.obj_n.matlabCodegenIsDeleted = false;
    encoder_arduino_test_DW.obj_n.SampleTime =
      encoder_arduino_test_P.MATLABSystem2_SampleTime;
    encoder_arduino_test_DW.obj_n.isInitialized = 1L;
    enc_init(0.0, 20.0, 21.0);
    encoder_arduino_test_DW.obj_n.isSetupComplete = true;
...
{
  /* Terminate for MATLABSystem: '<S1>/MATLAB System2' */
  matlabCodegenHandle_matlabC_kv3(&encoder_arduino_test_DW.obj_n);
